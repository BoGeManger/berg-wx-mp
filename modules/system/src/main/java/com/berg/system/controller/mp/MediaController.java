...
public class MediaController extends AbstractController {
    ...
    public Result<Void> mediaUpload(@ApiParam(value = "文件",required = true) @RequestPart(value = "file") MultipartFile file,
        ...
        return success("请求成功",()->mediaService.mediaUpload(appId,mediaType,remark,file));
    }

    @ApiOperation("上传图片永久素材")
    @PostMapping(value = "mediaImgUpload",consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public Result<Void> mediaImgUpload(@ApiParam(value = "文件",required = true) @RequestPart(value = "file") MultipartFile file,
                                       @ApiParam(value = "微信公众号appId",required = true) @RequestParam(value = "appId") String appId,
                                       @ApiParam(value = "描述") @RequestParam(value = "remark",required = false)String remark){
        return success("请求成功",()->mediaService.mediaImgUpload(appId,remark,file));
    }

    @ApiOperation("上传文件永久素材")
    @PostMapping(value = "materialFileUpload",consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public Result<Void> materialFileUpload(@ApiParam(value = "文件",required = true) @RequestPart(value = "file") MultipartFile file,
                                          @ApiParam(value = "微信公众号appId",required = true) @RequestParam(value = "appId") String appId,
                                          @ApiParam(value = "素材类型(image 图片 voice 语音 video 视频 thumb 缩略图)",required = true) @RequestParam(value = "mediaType") String mediaType,
                                          ...
                                          @ApiParam(value = "描述") @RequestParam(value = "remark",required = false)String remark){
        return success("请求成功",()->mediaService.materialFileUpload(appId,mediaType,name,videoTitle,videoIntroduction,remark,file));
    }

    @ApiOperation("上传图文永久素材")
    @PostMapping(value = "materialNewsUpload")
    public Result<Void> materialNewsUpload(@RequestBody @Validated MpMaterialNewsUploadInVo input){
        return success("请求成功",()->mediaService.materialNewsUpload(input));
    }
    ...
    public Result<Boolean> materialNewsUpdate(@RequestBody @Validated MpMaterialNewsUpdateInVo input){
        return success("请求成功",()->mediaService.materialNewsUpdate(input));
    }

    @ApiOperation("删除永久素材")
    @PutMapping(value = "materialDelete")
    public Result<Boolean> materialDelete(@RequestBody @Validated MpMaterialDeleteInVo input){
        return success("请求成功",()->mediaService.materialDelete(input.getAppId(),input.getMediaId()));
    }
}
