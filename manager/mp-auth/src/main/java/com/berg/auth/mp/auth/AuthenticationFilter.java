...
public class AuthenticationFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws UnauthorizedException {
        ...
        AuthConstants authConstants = SpringUtil.getBean(AuthConstants.class);
        String[] anonUrl = StringUtils.splitByWholeSeparatorPreserveAllTokens(authConstants.getShiroUrls(), ",");
        boolean match = false;
        String thisUrl = httpServletRequest.getRequestURI();
        for (String u : anonUrl) {
            if (pathMatcher.match(u, thisUrl))
                match = true;
        }
